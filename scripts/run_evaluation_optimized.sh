...
declare -A MODELS_BATCH_SIZES=(
    # ["Qwen/Qwen3-Embedding-4B"]=16         # completed
    ["intfloat/multilingual-e5-large-instruct"]=32
    ["intfloat/multilingual-e5-large"]=32
    # ["BAAI/bge-m3"]=32                      # completed
    # ["Snowflake/snowflake-arctic-embed-l-v2.0"]=32  # completed
    ["Lajavaness/bilingual-embedding-large"]=32
    # ["manu/bge-fr-en"]=32                   # completed
    # ["google/embeddinggemma-300m"]=64       # completed
    ...
    # ["Gameselo/STS-multilingual-mpnet-base-v2"]=128 # completed
    # ["Omartificial-Intelligence-Space/Arabic-MiniLM-L12-v2-all-nli-triplet"]=128  # completed
)

SUMMARY_FILE="$SCRIPT_DIR/results/all_models_summary.csv"
echo "model_name,k,recall,eir,ndcg,mrr" > "$SUMMARY_FILE"

echo "ðŸš€ Optimized Evaluation: 28 Models Ã— 6 TopK"
echo " Dataset: 1000 queries | vCPUs: 30 workers"
echo "â±ï¸  Estimated: 1-1.5 hours"
...
for MODEL in "${!MODELS_BATCH_SIZES[@]}"; do
    ...
    # Check if all predictions exist
    ALL_EXIST=true
    ...
    if [ "$ALL_EXIST" = true ]; then
        echo "â­ï¸  Predictions exist, calculating metrics..."
    else
        echo "ðŸ” Running FAISS retrieval (once for all k)..."
        python "$SCRIPT_DIR/rageval/evaluation/evaluate_dense_biencoder_faiss.py" \
            ...
            --device $DEVICE \
            --use_gpu_faiss 2>&1 | grep -E "âœ…|ðŸ“¥|ðŸ”½|ðŸ’¾"
    ...
    # Calculate metrics for ALL k values in parallel
    echo " Calculating metrics for all k values (30 workers)..."
...
print(f"  k={results['k']:3d}: Recall={results.get('recall',0):.4f} EIR={results.get('eir',0):.4f} NDCG={results.get('ndcg',0):.4f} MRR={results.get('mrr',0):.4f}")
EOF
    done
    
    # Clean predictions AFTER all metrics calculated
    echo "ðŸ§¹ Cleaning predictions..."
    ...
    echo "â±ï¸  Elapsed: $((ELAPSED/60))m | Avg: $((AVG/60))m/model | ETA: ${ETA}m"
...
echo "â±ï¸  Total: $((DURATION/60))m ($((DURATION/3600))h $((DURATION%3600/60))m)"
echo "=========================================="
echo ""
column -t -s',' "$SUMMARY_FILE" | head -n 31
echo ""
echo "ðŸ“ Full results: cat $SUMMARY_FILE"
